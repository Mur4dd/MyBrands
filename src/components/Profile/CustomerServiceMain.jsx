import {customerservices} from '../../utils/Profile/Constants'
            ...
            {data.faqs.map((faq, index) => {
                ...
                return (
                    <div className='flex flex-col items-between justify-start w-full border border-gray-100 p-3 py-5' key={index}>
                        ...
                        <div className={`accordionanimate ${isOpen ? 'open' : 'closed'}  ${faq.title == "Ölçü cədvəli" ? 'flex-row justify-around ' : 'flex-col'} flex `}>
                            {
                               isOpen &&  
                                    faq.answer.map((answer, index) => {
                                        return (
                                            <p className={`accordionanimate text-[#9B96B7] text-[14px] max-w-[500px] mt-[15px] ${faq.title == "Ölçü cədvəli" && 'font-semibold text-black text-25px[]'}`}>
                                                {answer}
                                            </p>
                                        )
                                    })
